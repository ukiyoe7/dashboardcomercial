                        ...
                        INNER JOIN (SELECT ZOCODIGO FROM ZONA 
                         WHERE ZOCODIGO IN (26))Z ON 
                          E.ZOCODIGO=Z.ZOCODIGO WHERE ENDFAT='S')A ON C.CLICODIGO=A.CLICODIGO
                           ...
                           TBPRECO AS (SELECT TBPCODIGO,TBPDESCRICAO FROM TABPRECO WHERE TBPCODIGO IN (102,202,104,103,301,302,303,304,305,307,308,313,3660,3661))
                      ...
                      WHEN TBPDESCRICAO='AVANCE' THEN 'AVANCE'
                       WHEN TBPDESCRICAO='INSIGNE' THEN 'INSIGNE'
                        WHEN TBPDESCRICAO='PRIMO' THEN 'PRIMO'
                         WHEN TBPDESCRICAO='MARCAS PROPRIAS CLIENTES' THEN 'MARCAS CLIENTES'
                          WHEN TBPDESCRICAO='MARCAS PROPRIAS CLIENTES' THEN 'MARCAS CLIENTES'
