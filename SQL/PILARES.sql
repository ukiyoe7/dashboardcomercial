WITH  CLI AS (SELECT DISTINCT C.CLICODIGO,GCLCODIGO FROM CLIEN C
              INNER JOIN (SELECT CLICODIGO,E.ZOCODIGO,ENDCODIGO FROM ENDCLI E
                          INNER JOIN (SELECT ZOCODIGO,ZODESCRICAO FROM ZONA 
                                      WHERE ZOCODIGO IN (26))Z ON 
                          E.ZOCODIGO=Z.ZOCODIGO WHERE ENDFAT='S')A ON C.CLICODIGO=A.CLICODIGO
                            WHERE CLICLIENTE='S'),

FIS AS (SELECT FISCODIGO FROM TBFIS WHERE (FISTPNATOP IN ('V','R','SR') OR FISCODIGO IN ('5.91V','6.91V','5.910','6.910'))),
...
PROSING AS (SELECT PROCODIGO FROM PRODU WHERE PRODESCRICAO LIKE '%PROSING%')
...
WHEN MRCL.PROCODIGO IS NOT NULL THEN 'MARCAS CLIENTES'
WHEN PRS.PROCODIGO IS NOT NULL THEN 'PROSING'
...
LEFT JOIN PROSING PRS ON PRS.PROCODIGO=PD.PROCODIGO
